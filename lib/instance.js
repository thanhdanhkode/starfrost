...
class InstanceManager {
  ...
  checkDirectory() {
    try {
      const instanceDir = join(__dirname, '..', '.starfrost', 'instances');
  ...
  info({ instanceId }) {
    const instanceInfo = this.instances.find((instance) => instance.instanceId === instanceId);
    return instanceInfo;
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.fastify.database.db.run(sql, [instanceId, instanceName, instanceOwner, instancePath], (error) => {
        if (error) {
          ...
          this.fastify && this.fastify.log.info(`Creating instance with name: ${instanceName}`);
